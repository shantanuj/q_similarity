...
embed_layer = Embedding(len(word_embeddings), embedding_dims, weights = [word_embeddings], input_length = max_seq_length, trainable = False)#trainable = False)
...
concat = keras.layers.concatenate([q1_enc, q2_enc])
dense_1 = Dense(64, activation='relu')(concat)
concat_l1 = keras.layers.concatenate([dense_1, l1_dist])
dense_out = Dense(1, activation="sigmoid", name="output")(concat_l1)


final_model = Model(inputs = [q1_placeholder, q2_placeholder], outputs = [dense_out]) 
...
with open("bilstm_fcc.json",'w') as json_file:
    json_file.write(model_json)
final_model.save_weights("bilstm_fcc.h5")
