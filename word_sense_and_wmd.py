...
'''The advantage of this mechanism is that it is unsupervised and can be used as additional features for deep learning based methods.'''
try:
...
def get_stop_words(directory="./stop_words.txt"):
    try:
...
def get_similarity(q1, q2, print_out=False):
    ...
    try:
        if (word_vectors_loaded):
        '''Perhaps use a classifier to determine below weights'''
